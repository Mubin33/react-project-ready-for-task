...
const onSubmit =async (data) => {
      const res = await axios.post("http://127.0.0.1:8000/api/password-reset/confirm/", data)
      if(res.data.status) {
        alert("Successfull")
      }
    ...
    <div> 
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='w-96 mx-auto shadow-sm p-2'> 
            ...
            <div className='flex flex-col space-y-2 mt-2'>
                ...
                <input className='border p-2 rounded-sm' value={token} type="text" placeholder='token' {...register('token')}/>
            </div>
